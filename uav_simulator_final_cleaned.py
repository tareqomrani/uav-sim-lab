...
with st.form("uav_form"):
    ...
    payload_weight_g = st.number_input("Payload Weight (g)", min_value=0, max_value=max_lift, value=int(max_lift * 0.5))
...
if submitted:
    try:
        ...
        if drone_model != "Custom Build" and UAV_PROFILES[drone_model]["power_system"] == "Hybrid":
            ...
            drag_factor = 1 + (flight_speed_kmh / 100) + (altitude_m / 10000)
            total_draw = base_draw * drag_factor * (1 + total_weight_kg / 1000)
        else:
            total_draw = total_power_draw * efficiency_penalty

        climb_energy_j = max(0, total_weight_kg * 9.81 * elevation_gain_m)
        battery_capacity_wh -= climb_energy_j / 3600

        flight_time_minutes = max(0, min((battery_capacity_wh / total_draw) * 60, 120))
