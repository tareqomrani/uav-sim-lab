...
if "simulate_flight" not in st.session_state:
    st.session_state.simulate_flight = False
...
with tab1:
    ...
    if st.button("Simulate Flight"):
        ...
        st.success("Simulation initialized! Please click the 'Simulate Flight' tab above to begin.")

# Simulation tab
with tab2:
    st.subheader("Flight Simulation Preview")

    if st.session_state.simulate_flight:
        st.success("Simulation running with parameters from Estimator.")
