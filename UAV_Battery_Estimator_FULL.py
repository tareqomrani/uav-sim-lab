...
with st.expander("📍 Mission Planning (Experimental)"):
    ...
    for i in range(mission_legs):
        ...
        dist = st.number_input(f"Distance for Leg {i+1} (km)", min_value=0.1, value=2.0, key=f"dist_{i}")
        alt = st.number_input(f"Altitude for Leg {i+1} (m)", min_value=0, max_value=5000, value=100, key=f"alt_{i}")
        legs.append((dist, alt))
    if st.button("Estimate Mission Plan"):
        total_mission_dist = sum([leg[0] for leg in legs])
        st.success(f"Total planned mission distance: **{total_mission_dist:.2f} km**")
...
with st.form("uav_form"):
    ...
    default_payload = int(profile["max_payload_g"] * 0.5)
...
if submitted:
    try:
        ...
        insert_thermal_and_fuel_outputs(total_draw, profile, flight_time_minutes, temperature_c, ir_shielding, delta_T)

        # Thermal signature plot
        time_range = np.linspace(0, flight_time_minutes, num=100)
        thermal_curve = delta_T * (1 + 0.05 * np.sin(2 * np.pi * time_range / flight_time_minutes))
        fig, ax = plt.subplots()
        ax.plot(time_range, thermal_curve)
        ...
        report = {
            ...
            "Payload Weight (g)": payload_weight_g,
            "Flight Speed (km/h)": flight_speed_kmh,
            "Estimated Flight Time (min)": round(flight_time_minutes, 1),
            "Estimated Max Distance (km)": round((flight_time_minutes / 60) * flight_speed_kmh, 2),
            "Battery Used (Wh)": round(battery_capacity_wh, 2),
            "Thermal Signature ΔT": round(delta_T, 1),
            "Risk Rating": thermal_risk_rating(delta_T),
            "Fuel Used (L)": calculate_fuel_consumption(total_draw, flight_time_minutes / 60)
                if profile["power_system"].lower() == "hybrid" else None
        }
        st.download_button("📄 Download Simulation Report (JSON)", json.dumps(report, indent=2), file_name="uav_simulation_report.json")
