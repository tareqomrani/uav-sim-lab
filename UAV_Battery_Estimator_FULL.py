...
if submitted:
    try:
        ...
        # Define mission phases with multipliers
        default_phases = [
            {"phase": "Takeoff/Climb", "duration_min": 5, "power_factor": 1.25},
            {"phase": "Cruise", "duration_min": 30, "power_factor": 1.0},
            {"phase": "Loiter", "duration_min": 15, "power_factor": 1.1},
            {"phase": "Ingress/Egress", "duration_min": 10, "power_factor": 1.15},
            {"phase": "Landing", "duration_min": 5, "power_factor": 0.8},
        ]
        phase_df = pd.DataFrame(default_phases)
        st.dataframe(phase_df)

        # Compute consumption phase by phase
        rows = []
        remaining_energy = (battery_capacity_wh if profile["power_system"]=="Battery" else
                            (ice_params["fuel_tank_l"]*ice_params["fuel_density_kgpl"]*1000 if use_ice_branch else 0))
        for ph in default_phases:
            dur = ph["duration_min"]
            pf  = ph["power_factor"]
            if profile["power_system"]=="Battery":
                draw = detail.get("total_draw_W", 200) * pf
                used = (draw * (dur/60.0)) / 3600.0 * 1000  # Wh
                remaining_energy -= used
            else:
                draw = detail.get("P_total_W", 1000) * pf
                lph  = bsfc_fuel_burn_lph(draw, ice_params["bsfc_gpkwh"], ice_params["fuel_density_kgpl"])
                used = lph * (dur/60.0)
                remaining_energy -= used
            rows.append({
                "Phase": ph["phase"],
                "Duration (min)": dur,
                "Avg Power Draw (W)": round(draw,1),
                "Energy/Fuel Used": round(used,2),
                "Remaining": round(max(0,remaining_energy),2)
            })
        phase_results = pd.DataFrame(rows)
        st.dataframe(phase_results)

        # Graphs of remaining and consumption
        fig1, ax1 = plt.subplots()
        ax1.plot(phase_results["Phase"], phase_results["Remaining"], marker="o")
        ax1.set_ylabel("Remaining Energy/Fuel")
        ax1.set_title("Phase Remaining Energy/Fuel")
        st.pyplot(fig1)

        fig2, ax2 = plt.subplots()
        ax2.bar(phase_results["Phase"], phase_results["Energy/Fuel Used"])
        ax2.set_ylabel("Used per Phase")
        ax2.set_title("Phase Consumption")
        st.pyplot(fig2)

        # Export graphs as PNG
        buf = io.BytesIO()
        fig1.savefig(buf, format="png"); buf.seek(0)
        st.download_button("⬇️ Download Energy Graph (PNG)", buf, "phase_energy.png","image/png")

        # CSV/JSON export
        st.download_button(
            "⬇️ Download Phase Results (CSV)",
            data=phase_results.to_csv(index=False).encode("utf-8"),
            file_name="phase_results.csv",
            mime="text/csv"
        )
        st.download_button(
            "⬇️ Download Phase Results (JSON)",
            data=phase_results.to_json(orient="records", indent=2),
            file_name="phase_results.json",
        ...
        elements = [Paragraph("UAV Phase Simulation Report", styles['Title']), Spacer(1,12)]
        for _,row in phase_results.iterrows():
            elements.append(Paragraph(f"{row['Phase']}: {row['Duration (min)']} min, "
                                      f"Draw={row['Avg Power Draw (W)']} W, "
                                      f"Used={row['Energy/Fuel Used']}, "
                                      f"Remaining={row['Remaining']}", styles['Normal']))
            elements.append(Spacer(1,6))
        doc.build(elements)
        pdf_buf.seek(0)
        st.download_button("⬇️ Download Phase Report (PDF)", pdf_buf, "phase_report.pdf","application/pdf")
