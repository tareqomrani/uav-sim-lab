...
with open('uav_profiles.json', 'r') as f:
    UAV_PROFILES = json.load(f)
...
with st.form("uav_form"):
    ...
    simulate_failure = st.checkbox("Enable Failure Simulation (experimental)")
        ...
        simulate_live = st.checkbox("Run Real-Time Simulation", value=True)
    ...
    try:
        ...
        if profile["power_system"] == "Battery":
                    if flight_mode == "Hover":
                        ...
                        total_draw = base_draw + 0.5 * 1.225 * 0.3 * 0.1 * (flight_speed_kmh / 3.6) ** 2  # drag model
        else:
        ...
        if elevation_gain_m > 0:
            ...
            initial_battery_wh = battery_capacity_wh
        ...
        if simulate_live:
            ...
            battery_pct = max(0, (battery_remaining / initial_battery_wh) * 100)
            ...
            battery_remaining = battery_capacity_wh - (total_steps * battery_per_step)
            battery_pct = max(0, (battery_remaining / initial_battery_wh) * 100)
            time_remaining = 0
            bars = int(battery_pct // 10)
            gauge.markdown(f"**Battery Gauge:** `[{'|' * bars}{' ' * (10 - bars)}] {battery_pct:.0f}%`")
            timer.markdown(f"**Elapsed:** {(flight_time_minutes * 60):.0f} sec\u2003**Remaining:** 0 sec")
            status.markdown(f"**Battery Remaining:** {battery_remaining:.2f} Wh  \\n**Power Draw:** {total_draw:.0f} W")
