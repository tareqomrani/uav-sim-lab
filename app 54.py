...
altitude_m = st.number_input("Flight Altitude (m)", min_value=0, max_value=5000, value=0, help="Higher altitudes reduce air density and increase power needs.")
    ...
    if (elevation_gain_m > 150 and drone_model != "Custom Build") or (drone_model == "Custom Build" and elevation_gain_m > 250):
        st.warning("Warning: This elevation gain may exceed typical UAV limits. Confirm thrust and altitude limits for this model.")
...
if submitted:
    ...
    # Adjust for altitude-related air density drop
...
st.caption(f"Air density factor at {altitude_m} m: {air_density_factor:.2f}")
    ...
    if battery_capacity_wh <= 0:
        st.info("Simulation stopped: climb energy exceeded total battery capacity.")
        st.stop()
