...
if submitted:
    try:
        ...
        efficiency_penalty = 1.4 if load_ratio > 1.0 else 1.25 if load_ratio > 0.9 else 1.1 if load_ratio > 0.7 else 1.0

        if drone_model != "Custom Build" and UAV_PROFILES[drone_model]["power_system"] == "Hybrid":
            draw = UAV_PROFILES[drone_model]["draw_watt"]
            speed_factor = 0.01 * flight_speed_kmh
            altitude_factor = 0.001 * altitude_m
            mass_factor = 0.003 * total_weight_kg
            total_draw = draw * (1 + speed_factor + altitude_factor + mass_factor)
        else:
